...
**FR-PPP-7** — **Replay protection.** Submitting the same `probe_id` to `POST /api/sm-probe/ack` more than once MUST return HTTP 409 (`{error: "replay", probe_id, existing}`); the first write wins. The `provenance_assertions.probe_id` column carries a `UNIQUE` constraint enforced at the SQLite level, so duplicate submissions raise `sqlite3.IntegrityError` even when the endpoint check is bypassed. Cassette coverage (FR-PPP-6) MUST include at least one replay scenario asserting the 409 path. Test floor: `tests/test_audit_probe_replay.py::test_endpoint_409_on_replay` + `::test_db_unique_constraint_raises`.
...
**FR-PPP-12** — **Layer-3 decoy registration.** SM MUST expose `POST /api/sm-decoy/register` (operator-triggered or auto-fired at SM boot) that registers a synthetic JSONL path that is never written by SM. The path SHAPE convention is `~/.claude/projects/sm-decoy-control/<decoy-uuid>.jsonl` where `<decoy-uuid>` is a fresh `secrets.token_hex(16)` and `sm-decoy-control` is a sentinel literal slug; the endpoint generates the path when the body omits one. The registration row lands in the additive `provenance_decoys` WAL table (`probe_id UNIQUE`, `jsonl_path UNIQUE`, `triggered_at` nullable); the HMAC sig binds `{probe_id, jsonl_path, registered_at}` (sig_v=1, endpoint-integrity per FR-PPP-2). Re-register on the same path is idempotent — `INSERT … ON CONFLICT(jsonl_path) DO NOTHING` returns the existing row's metadata with `first_write=false`.

**FR-PPP-13** — **Layer-3 hallucination detector.** When ANY code path inside `jsonl_tail` / `session_watcher` / `governance` reports a parsed record whose `jsonl_path` matches a registered decoy (per `MessageBus.is_registered_decoy_path`), SM MUST emit an `audit.hallucination_detected` envelope and stamp `provenance_decoys.triggered_at`. The envelope shape is `{probe_id, jsonl_path, detected_at, hmac_sig}`; the sig is server-stamped at detection time (sig_v=1). The single-emit invariant is enforced at the WAL row level via `mark_decoy_triggered`'s `WHERE triggered_at IS NULL` clause — a second parsed-record on the same decoy is a no-op envelope-wise. The dashboard renders this envelope via the `.hitl-item-hallucination` row variant (RED border + alarm icon); the row does NOT auto-clear — operator-dismiss only (this is a parser-correctness alarm, not an operator-attestation prompt). The P2 observer's existing `_is_sm_originated` filter MUST fire before any decoy match (defense in depth: SM-originated transcripts never trigger).

**FR-PPP-14** — **Layer-3 self-monitor hard guard + cassette coverage.** `session_watcher.build_audit_probe_candidates`'s `sm_brain_id` keyword argument MUST be MANDATORY (no `None` default). Calling without a resolved `SM_OWN_SESSION_ID` raises `RuntimeError("SM_OWN_SESSION_ID required for audit-probe candidate build")`. The P1a defense-in-depth optional kwarg graduated to mandatory in P3 so the failure mode flips from silent ("filter not applied") to loud (immediate exception at the call site) per `feedback_no_self_monitor.md`. All real callers MUST resolve the env var at call time (`dashboard/server.py:/api/sm-probe` raises HTTP 500 when the env var is unset). Cassette coverage: `tools/cassette_record._record_ppp_envelopes` MUST record one `audit.hallucination_detected` envelope + one `provenance_decoys` row same-cycle alongside the Layer-1/2 surface (FR-PPP-6, FR-PPP-11). Decoy envelopes ride the same `--skip-ppp-pump` opt-out — Layer 3 is part of the PPP pump, not a separate cassette section.

Originating task: `docs/prompts/v2.1-orchestration/phase-3-ppp-negative-control.md`. Cycle scope: `docs/v2.1-p3-scope.md`.
