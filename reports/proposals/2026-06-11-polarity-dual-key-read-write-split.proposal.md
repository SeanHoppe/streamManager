...
- **Disposition:** PROPOSAL (governance-doc change) → **REVISE → PARTLY DONE (`eb52689`).** Verified research (workflow `w7npngehj`, 2026-06-11) confirmed the split is sound but the wording was overstated and two real gaps exist. **Actioned in `eb52689`:** (a) softened the CLAUDE.md (`e713b12`) bullet + `rl/ope.py` + `rl/corpus_augment.py` comments to "project_slug = durable read key; session_id = write gate + cheap read backstop"; (b) two-reader policy — added the read-time session backstop to `rl/ope.load_episodes_from_db` so all readers match (defence-in-depth); (c) added the missing NULL-slug + SM-self read-exclusion tests for ope + corpus_augment. **Still TRACKED (operator deferred):** the `BRIDGE_SM_SELF_SESSION_ID` fail-loud precondition, and the cassette/probe synthetic-path slug guard. See **Verified-research update** at the bottom.
