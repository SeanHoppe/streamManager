...
Sean's persistent coding style, approach nuances, and do-not-do rules live
in `./MEMORY.md` (checked in, project-root). **Read it at session start and
update it whenever Sean teaches a new preference.** It is distinct from the
auto-memory store under
`C:\Users\SeanHoppe\.claude\projects\C--Users-SeanHoppe-vs-streamManager\memory\`
(per-topic, machine-managed, not checked in).
